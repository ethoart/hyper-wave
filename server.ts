...
const userSchema = new mongoose.Schema({
  ...
  role: {
    type: String,
    enum: ["super_admin", "admin", "pro", "user"],
    default: "user",
  },
...
const tradeSignalSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ["pending", "win", "loss", "invalidated", "expired", "closed"],
    default: "pending",
  },
  pnlPercent: Number,
  realizedPnl: Number,
...
const userTradeSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ["pending", "live", "win", "loss", "expired", "closed"],
    default: "pending",
  },
...
const scriptItemSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ["indicator", "strategy", "signal"],
    default: "indicator",
  },
...
const ScriptItem: any =
  mongoose.models.ScriptItem || mongoose.model("ScriptItem", scriptItemSchema);
...
const ChartDrawing: any =
  mongoose.models.ChartDrawing ||
  mongoose.model("ChartDrawing", chartDrawingSchema);
...
async function startServer() {
  ...
  if (process.env.MONGO_URI) {
    try {
      ...
      const superAdminEmail =
        process.env.SUPER_ADMIN_EMAIL || "admin@admin.com";
      const superAdminPassword = process.env.SUPER_ADMIN_PASSWORD || "admin123";
      const adminExists = await (User as any).findOne({
      ...
      if (
        ...
        console.error(
          "CRITICAL: Mongoose could not connect to your MongoDB cluster.",
        );
        console.error(
          "If you are using MongoDB Atlas, this usually means your IP address",
        );
        console.error(
          "is not whitelisted. Go to Atlas Security -> Network Access and",
        );
        console.error(
          "add 0.0.0.0/0 (allow access from anywhere) to connect from this app.",
        );
        console.error(
          "======================================================================",
        );
    ...
    console.warn(
      "WARNING: MONGO_URI is not set. Database features will error out.",
    );
  }
  ...
  if (process.env.MONGO_URI) {
    try {
      ...
      console.error(
        "Failed to initialize MongoStore. Defaulting to MemoryStore.",
        err,
      );
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    if (!isDbConnected) {
      ...
      if (email === superAdminEmail && password === superAdminPassword) {
        ...
        console.warn(
          "MongoDB not connected: Using fallback super admin login.",
        );
        return res.json({
          user: {
            email,
            role: "admin",
            message: "Logged in using fallback memory mode",
          },
        });
      }
      return res
        ...
        .json({
          error:
            "Database is not connected. If you have MONGO_URI set, ensure MongoDB is running.",
        });
    }
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    if (!isDbConnected)
      return res
        .status(500)
        .json({
          error: "Database is not connected. User registration is disabled.",
        });
    try {
  ...
  app.put("/api/users/settings", authMiddleware, async (req: any, res) => {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only PRO users can update these settings" });
    }
    try {
      const { binanceApiKey, binanceSecretKey, useCustomAlgo, pineCode } =
        req.body;
      const user = await (User as any)
        .findByIdAndUpdate(
          req.user._id,
  ...
  app.get("/api/drawings/:symbol", authMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      const drawing = await ChartDrawing.findOne({
  ...
  app.post("/api/drawings/:symbol", authMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      let drawing = await ChartDrawing.findOne({
        userEmail: req.user.email,
        symbol,
        interval: interval || "1d",
      });
      if (drawing) {
        ...
        drawing = await ChartDrawing.create({
          userEmail: req.user.email,
          symbol,
          interval: interval || "1d",
          drawings,
    ...
    async (req: any, res) => {
      ...
      try {
        ...
        if (!drawing)
          return res
            .status(404)
            .json({ error: "Save your chart first before sharing." });
  ...
  app.get("/api/market/depth", async (req: any, res) => {
    ...
    try {
      const response = await axios.get(
        `https://fapi.binance.com/fapi/v1/depth?symbol=${symbol.toUpperCase()}&limit=${limit}`,
      );
  ...
  app.post("/api/trade/auto", async (req: any, res) => {
    try {
      ...
      const { symbol, trend, entry, target, stopLoss, amount, setupData } =
        req.body;

      const activeGlobalTrends = await TradeSignal.distinct("trend", {
        status: { $in: ["pending", "live"] },
      });
      if (
        activeGlobalTrends.length > 0 &&
        !activeGlobalTrends.includes(trend)
      ) {
        return res.json({
          success: false,
          message: `Trade rejected: Portfolio is currently committed to ${activeGlobalTrends[0]} direction. Cannot mix long and short trades.`,
        });
      }
      ...
      if (projectedProfit < 2) {
        return res.json({
          success: false,
          message: `Trade rejected: projected profit $${projectedProfit.toFixed(2)} is less than $2.00`,
        });
      }
      ...
      res.json({
        success: true,
        message: "Trade signal queued. Waiting for entry.",
      });
    } catch (err) {
  ...
  app.get("/api/trades", authMiddleware, async (req: any, res) => {
    try {
      ...
      const pendingTrades = await TradeSignal.find({ status: "pending" })
        .sort({ timestamp: -1 })
        .limit(20);
      const closedSignals = await TradeSignal.find({
        status: { $ne: "pending" },
      })
        .sort({ resolvedAt: -1 })
        .limit(50);
      const closedUserTrades = await UserTrade.find({
        userId: req.user._id,
        status: { $ne: "live" },
      })
        .sort({ resolvedAt: -1 })
      ...
      const closed = [
        ...closedSignals.map((sig) => ({
          ...
          type: "auto",
          realizedPnl:
            sig.realizedPnl !== undefined
              ? sig.realizedPnl
              : sig.pnlPercent
                ? (sig.amount *
                    (sig.setupData?.leverage || 10) *
                    sig.pnlPercent) /
                  100
                : 0,
        ...
        ...closedUserTrades.map((ut) => ({
          ...
          pnlPercent: ut.realizedPnl
            ? (ut.realizedPnl / (ut.amount * (ut.leverage || 10))) * 100
            : 0,
        })),
      ]
        .sort(
          (a: any, b: any) =>
            new Date(b.resolvedAt || b.timestamp).getTime() -
            new Date(a.resolvedAt || a.timestamp).getTime(),
        )
        .slice(0, 50);
      ...
      try {
        ...
        const userPaperTrades = await UserTrade.find({
          userId: req.user._id,
          status: "live",
        ...
        for (const pt of userPaperTrades) {
          try {
            const tickRes = await axios.get(
              `https://fapi.binance.com/fapi/v1/ticker/price?symbol=${pt.symbol}`,
            );
      ...
      const pending = await Promise.all(
        pendingTrades.map(async (trade: any) => {
          let t = trade.toObject();
          if (t.binanceOrderId) {
            try {
              const tickerRes = await axios.get(
                `https://fapi.binance.com/fapi/v1/ticker/price?symbol=${t.symbol}`,
              );
      ...
      const userTradesForStats = await UserTrade.find({
        userId: req.user._id,
        status: "closed",
      });
      const stats = {
        ...
        losses: userTradesForStats.filter((t: any) => t.realizedPnl <= 0)
          .length,
      };
  ...
  app.get("/api/admin/subtract", async (req: any, res) => {
    ...
    if (!cfg) {
      cfg = new EngineConfig({ id: "global", autoBotBalance: 100 });
    }
    cfg.autoBotBalance = Math.max(0, (cfg.autoBotBalance || 100) - 90);
    cfg.params = { retrace2: 0.786, ext3: 2.618, retrace4: 0.382 };
    cfg.insights =
      "Decreased risk tolerance due to recent losses. Implementing stricter invalidation rules and deeper retracements (W2 > 0.786) to filter fakeouts out.";
    await cfg.save();
  ...
  app.get("/api/admin/reset-budget", async (req: any, res) => {
    ...
    try {
      ...
      await TradeSignal.updateMany(
        { status: { $in: ["pending", "live"] } },
        {
          $set: {
            status: "invalidated",
            resolvedAt: new Date(),
            closeReason: "Budget reset by logic",
          },
        },
      );
    ...
    try {
      ...
      const data = response.data.filter(
        (d: any) =>
          ...
          d.symbol !== "USUSDT" &&
          !["USDC", "FDUSD", "TUSD", "BUSD", "EUR", "USDP"].some((s) =>
            d.symbol.includes(s),
          ) &&
  ...
  app.post("/api/trade/place", authMiddleware, async (req: any, res) => {
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only PRO users and Admins can place trades." });
    }
    ...
    try {
      ...
      const qty =
        (parseFloat(amount) / executedPrice) * parseFloat(leverage || "1");

      if (req.user.role === "pro") {
        orderTrackingId = `paper_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
      } else {
        try {
          const apiRes = await placeBinanceTrade(
            symbol,
            side,
            qty,
            "MARKET",
          ...
          orderTrackingId = apiRes.orderId
            ? apiRes.orderId.toString()
            : apiRes.clientOrderId;
        } catch (e: any) {
          console.warn(
            `[Binance] Manual trade failed, falling back to Paper:`,
            e.message,
          );
          orderTrackingId = `paper_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
        }
      ...
      res.json({
        success: true,
        message: `Trade executed!${orderTrackingId.startsWith("paper") ? " (Paper)" : ""}`,
  ...
  app.post("/api/trade/tpsl", authMiddleware, async (req: any, res) => {
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only PRO users and Admins can manage TP/SL" });
    }
    ...
    try {
      ...
      if (!apiKey || !secretKey)
        return res
          .status(500)
          .json({ error: "Configure Binance API keys in wallet settings" });

      const baseEndpoint = isTestnet
        ? "https://testnet.binancefuture.com"
        : "https://fapi.binance.com";
      const getSig = (qs: string) =>
      ...
      await axios
        .delete(
          `${baseEndpoint}/fapi/v1/allOpenOrders?${cancelQs}&signature=${getSig(cancelQs)}`,
      ...
      if (sl) {
        ...
        await axios.post(
          `${baseEndpoint}/fapi/v1/order?${qs}&signature=${getSig(qs)}`,
          null,
          { headers: { "X-MBX-APIKEY": apiKey } },
        );
      }
  ...
  app.post("/api/trade/close", authMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      if (userPaperTrade) {
        ...
        return res.json({
          success: true,
          message: `Closed Paper position. Realized PnL: $${realizedPnl.toFixed(2)}`,
        });
      }

      const user = await User.findById(req.user._id);
      const binanceTrade = await UserTrade.findOne({
        userId: req.user._id,
        symbol,
        status: "live",
      });
  ...
  app.post("/api/trade/close_auto", authMiddleware, async (req: any, res) => {
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only Admins can manage auto trades" });
    }
  ...
  app.post("/api/users/upgrade-pro", authMiddleware, async (req: any, res) => {
    if (!isDbConnected) {
      return res
        ...
        .json({
          error:
            "Database is not connected. Upgrade is disabled in memory mode.",
        });
    }
    try {
  ...
  app.post("/api/analysis/generate", authMiddleware, async (req: any, res) => {
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only PRO users and Admins can generate analysis." });
    }
    ...
    if (!data || !Array.isArray(data) || data.length === 0) {
      return res
        .status(400)
        .json({ error: "Chart data is required for analysis." });
    }

    if (!process.env.GEMINI_API_KEY) {
      console.warn(
        "GEMINI_API_KEY not configured, falling back to Math Engine",
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (result.trend === "bullish") {
          if (result.exitPoint <= result.entryPoint)
            result.exitPoint = result.entryPoint * 1.05;
          if (result.stopLoss >= result.entryPoint)
            result.stopLoss = result.entryPoint * 0.95;
        } else if (result.trend === "bearish") {
          if (result.exitPoint >= result.entryPoint)
            result.exitPoint = result.entryPoint * 0.95;
          if (result.stopLoss <= result.entryPoint)
            result.stopLoss = result.entryPoint * 1.05;
        }
      } catch (aiError: any) {
        console.warn(
          "AI generation failed. Falling back to math engine output.",
          aiError.message,
        );
        ...
        result = {
          ...
          wavePoints: algoResult
            ? [
                {
                  time: algoResult.waves.start.time,
                  price: algoResult.waves.start.price,
                },
                {
                  time: algoResult.waves.w1.time,
                  price: algoResult.waves.w1.price,
                },
                {
                  time: algoResult.waves.w2.time,
                  price: algoResult.waves.w2.price,
                },
                {
                  time: algoResult.waves.w3.time,
                  price: algoResult.waves.w3.price,
                },
                {
                  time: algoResult.waves.w4.time,
                  price: algoResult.waves.w4.price,
                },
              ]
            : [],
        };
  ...
  app.get("/api/ml/history", async (req, res) => {
    ...
    try {
      ...
      const recent = await TradeSignal.find({ status: { $ne: "pending" } })
        .sort({ resolvedAt: -1 })
        .limit(10);
    ...
    try {
      ...
      for (const pair of topPairs) {
        try {
          ...
          else if (currentModel === "AUTO_OPTIMIZED")
            intervalsToScan = ["15m", "1h", "4h"];

          let bestAlgoResult = null;
          for (const scanInterval of intervalsToScan) {
            const klinesRes = await axios.get(
              `https://fapi.binance.com/fapi/v1/klines?symbol=${pair.symbol}&interval=${scanInterval}&limit=500`,
            );
            ...
            const algoResult = analyzeElliottWaves(
              chartData,
              scanInterval,
              mlParams,
            );
            if (algoResult && algoResult.trend !== "neutral") {
              ...
              if (
                !bestAlgoResult ||
                algoResult.gainPct > bestAlgoResult.gainPct
              ) {
                bestAlgoResult = algoResult;
                bestAlgoResult.currentPrice =
          ...
          if (algoResult && algoResult.trend !== "neutral") {
            ...
            const entryDiff =
              Math.abs(algoResult.entry - currentPrice) / currentPrice;
            ...
            let computedAmount = currentBudget * 0.1;
            if (computedAmount < 5) computedAmount = 5;
            if (computedAmount > currentBudget) computedAmount = currentBudget;
            const tradeAmountDollars =
              engineCfg?.tradeAmountFixed || computedAmount;

            const leverage = 10;
            const positionSizeUsdt = tradeAmountDollars * leverage;
            const priceDiff = Math.abs(algoResult.target - algoResult.entry);
            const projectedProfit =
              (positionSizeUsdt / algoResult.entry) * priceDiff;

            let slPrice = algoResult.stopLoss;
            let rawLossUsdt = 0;
            if (algoResult.trend === "bullish") {
              rawLossUsdt =
                (positionSizeUsdt / algoResult.entry) *
                (algoResult.entry - slPrice);
            } else {
              rawLossUsdt =
                (positionSizeUsdt / algoResult.entry) *
                (slPrice - algoResult.entry);
            }

            let recommendedAmount = tradeAmountDollars;

            console.log(
              `[AutoScanner] ${pair.symbol} | entryDiff: ${entryDiff} | profit: ${projectedProfit} | target: ${algoResult.target} | SL: ${algoResult.stopLoss} | entry: ${algoResult.entry}`,
            );
            if (
              entryDiff < 0.15 &&
              projectedProfit >= tradeAmountDollars * 0.05
            ) {
              // At least 5% ROE
              ...
              console.log(
                `[Auto-Scan] Found actionable trade for ${pair.symbol} (Proj. Pnl: $${projectedProfit.toFixed(2)})`,
              );
            }
          }
          ...
          console.warn(
            `[Auto-Scan] error scanning ${pair.symbol}`,
            (e as any).message,
          );
        }
      }
      ...
      if (foundAlerts.length > 0) {
        ...
        if (isDbConnected) {
          let engineCfg = await EngineConfig.findOne({ id: "global" });
          if (!engineCfg) {
            engineCfg = await EngineConfig.create({
              id: "global",
              autoBotBalance: 100,
            });
          }
          ...
          const activeCount = await TradeSignal.countDocuments({
            status: { $in: ["pending", "live"] },
          });
          const activeGlobalTrends = await TradeSignal.distinct("trend", {
            status: { $in: ["pending", "live"] },
          ...
          for (const alert of foundAlerts) {
            try {
              ...
              if (
                activeGlobalTrends.length > 0 &&
                !activeGlobalTrends.includes(alert.trend)
              ) {
                console.log(
                  `[Engine] Blocked ${alert.trend} on ${alert.symbol} because portfolio is already committed to ${activeGlobalTrends[0]} direction.`,
                );
              ...
              if (!existing && activeCount < 10) {
                ...
                if (computedAmount > currentBudget)
                  computedAmount = currentBudget;

                const tradeAmountDollars =
                  engineCfg.tradeAmountFixed || computedAmount;
                const leverage =
                  alert.setupData && alert.setupData.leverage
                    ? alert.setupData.leverage
                    : 10;
                ...
                if (alert.trend === "bullish") {
                  activeRawLoss =
                    (activePosSize / alert.entry) * (alert.entry - activeSl);
                } else {
                  activeRawLoss =
                    (activePosSize / alert.entry) * (activeSl - alert.entry);
                }
                ...
                const expiresAt = new Date(
                  Date.now() +
                    (alert.termStyle === "SHORT_TERM" ? 2 : 12) *
                      60 *
                      60 *
                      1000,
                );
  ...
  const runOutcomeEvaluator = async () => {
    ...
    isEvaluating = true;
    try {
      const pendingSignals = await TradeSignal.find({
        status: "pending",
      }).limit(50);
      for (const signal of pendingSignals) {
        try {
          ...
          const response = await axios.get(
            `https://fapi.binance.com/fapi/v1/klines?symbol=${signal.symbol}&interval=1m&limit=10`,
          );
          ...
          if (!signal.binanceOrderId) {
            if (
              signal.expiresAt &&
              Date.now() > new Date(signal.expiresAt).getTime()
            ) {
              outcome = "expired";
            } else {
              const diffPercent = Math.abs(price - signal.entry) / signal.entry;
              let isEntryHit = false;
              if (signal.trend === "bullish" && price <= signal.entry)
                isEntryHit = true;
              if (signal.trend === "bearish" && price >= signal.entry)
                isEntryHit = true;
              ...
              if (isEntryHit) {
                try {
                  ...
                  const leverage =
                    signal.setupData && signal.setupData.leverage
                      ? signal.setupData.leverage
                      : 10;
                  const positionSizeUsdt = tradeAmountDollars * leverage;
                  ...
                  try {
                    ...
                    orderId = apiRes.orderId
                      ...
                      : apiRes.clientOrderId;
                    console.log(
                      `[Binance] Executed Pending Entry: ${side} ${quantity} ${signal.symbol}`,
                    );
                  } catch (e: any) {
                    console.warn(
                      `[Binance] Could not place real trade, falling back to Paper Trade for ${signal.symbol}:`,
                      e.message,
                    );
                  ...
                  for (const pUser of proUsers) {
                    try {
                      if (!pUser.useCustomAlgo) {
                        ...
                        const userQuantity = +(userPosSizeUsdt / price).toFixed(
                          4,
                        );
                        try {
                          await setBinanceLeverage(
                        ...
                        if (pUser.role === "pro") {
                          ...
                          try {
                            const res = await placeBinanceTrade(
                              signal.symbol,
                              side,
                              userQuantity,
                              "MARKET",
                            ...
                            orderTrackingId =
                              ...
                              res.clientOrderId?.toString() ||
                              "unknown";
                          } catch (e: any) {
                            console.warn(
                              `[Binance] PRO Default Algo failed, falling back to Paper:`,
                              e.message,
                            );
                            orderTrackingId = `paper_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
                          }
          ...
          if (signal.binanceOrderId) {
            if (signal.trend === "bullish") {
              ...
              if (
                progress >= 0.85 &&
                signal.stopLoss <
                  signal.entry + (signal.target - signal.entry) * 0.7
              ) {
                signal.stopLoss =
                  signal.entry + (signal.target - signal.entry) * 0.7;
              ...
              if (currentPnl <= -(signal.amount || 10) * 1.2) {
                outcome = "loss";
                pnl = -120 / curLeverage; // -120% loss at 10x
                closeReason +=
                  "Maximum loss threshold hit (-120% of margin/Liquidation).";
              } else if (highPrice >= signal.target) {
                outcome = "win";
                pnl = ((signal.target - signal.entry) / signal.entry) * 100;
                closeReason += "Target price reached.";
              } else if (lowPrice <= signal.stopLoss) {
                let projectedPnl =
                  ((signal.stopLoss - signal.entry) / signal.entry) * 100;
                if (
                  Math.abs(
                    (projectedPnl / 100) * (signal.amount || 10) * curLeverage,
                  ) <= 0.6
                ) {
                  outcome = "closed";
                  pnl = projectedPnl;
                  closeReason += "Capital Protected (Break Even/Scratched).";
                } else {
                  outcome = signal.stopLoss > signal.entry ? "win" : "loss";
                  pnl = projectedPnl;
                  closeReason +=
                    signal.stopLoss > signal.entry
                      ? "Stopped out in profit (Trailing SL)."
                      : "Stop loss hit.";
                }
              }
              ...
              if (
                progress >= 0.85 &&
                signal.stopLoss >
                  signal.entry - (signal.entry - signal.target) * 0.7
              ) {
                signal.stopLoss =
                  signal.entry - (signal.entry - signal.target) * 0.7;
                closeReason += " Trailed SL to +70% profit. ";
              ...
              if (currentPnl <= -(signal.amount || 10) * 1.2) {
                ...
                closeReason +=
                  "Maximum loss threshold hit (-120% of margin/Liquidation).";
              } else if (lowPrice <= signal.target) {
                outcome = "win";
                pnl = ((signal.entry - signal.target) / signal.entry) * 100;
                closeReason += "Target price reached.";
              } else if (highPrice >= signal.stopLoss) {
                let projectedPnl =
          ...
          if (outcome !== "pending") {
            if (signal.binanceOrderId) {
              if (signal.binanceOrderId.toString().startsWith("paper_")) {
                console.log(
                  `[Paper Trade] Closed position for ${signal.symbol}`,
                );
              } else {
                try {
                  const res = await closeBinancePosition(signal.symbol);
                  console.log(
                    `[Binance] Closed position for ${signal.symbol}:`,
                    res.message,
                  );
                } catch (e: any) {
                  console.error(
                    `[Binance Close Error] ${signal.symbol}:`,
                    e.message,
                  );
                }
              }
            ...
            let engineCfg = await EngineConfig.findOne({ id: "global" });
            if (engineCfg) {
              engineCfg.autoBotBalance =
                (engineCfg.autoBotBalance || 100) + signal.realizedPnl;
            ...
            await signal.save();
            console.log(
              `[ML-Evaluator] Evaluated trade ${signal.symbol}: ${outcome.toUpperCase()} (${(pnl * leverage).toFixed(2)}% | $${(signal.realizedPnl || 0).toFixed(2)}) | New Budget: $${engineCfg?.autoBotBalance?.toFixed(2)}`,
            );
          }
        } catch (e) {}
      }
      ...
      for (const ut of liveUserTrades) {
        try {
          ...
          if (entry && target) {
            if (ut.side === "BUY" && target < entry) {
              ut.side = "SELL";
              await ut.save();
            } else if (ut.side === "SELL" && target > entry) {
              ut.side = "BUY";
              await ut.save();
            }
          }
          ...
          const response = await axios.get(
            `https://fapi.binance.com/fapi/v1/klines?symbol=${ut.symbol}&interval=1m&limit=10`,
          );
          if (!response.data || response.data.length < 2) continue;
          ...
          if (ut.side === "BUY") {
            ...
            if (
              userProgress >= 0.85 &&
              ut.stopLoss < entry + (target - entry) * 0.7
            ) {
              ut.stopLoss = entry + (target - entry) * 0.7;
              closeReason += " Trailed SL to +70% profit. ";
              updated = true;
            ...
            if (currentPnl <= -20) {
              ...
              outcome = "win";
              realizedPnl =
                ((target - entry) / entry) * ut.amount * (ut.leverage || 10);
              closeReason += "Target price reached.";
            } else if (ut.stopLoss && lowPrice <= ut.stopLoss) {
              let calcPnl =
                ((ut.stopLoss - entry) / entry) *
                ut.amount *
                (ut.leverage || 10);
              if (Math.abs(calcPnl) <= 0.6) {
                ...
                outcome = ut.stopLoss > entry ? "win" : "loss";
                realizedPnl = calcPnl;
                closeReason +=
                  ut.stopLoss > entry
                    ? "Stopped out in profit (Trailing SL)."
            ...
            if (
              userProgress >= 0.85 &&
              ut.stopLoss > entry - (entry - target) * 0.7
            ) {
              ut.stopLoss = entry - (entry - target) * 0.7;
              closeReason += " Trailed SL to +70% profit. ";
              updated = true;
            ...
            if (currentPnl <= -20) {
              ...
              outcome = "win";
              realizedPnl =
                ((entry - target) / entry) * ut.amount * (ut.leverage || 10);
              closeReason += "Target price reached.";
            } else if (ut.stopLoss && highPrice >= ut.stopLoss) {
              let calcPnl =
                ((entry - ut.stopLoss) / entry) *
                ut.amount *
                (ut.leverage || 10);
              if (Math.abs(calcPnl) <= 0.6) {
                outcome = "closed";
          ...
          if (outcome !== "live") {
            if (ut.binanceOrderId && !ut.binanceOrderId.startsWith("paper_")) {
              ...
              if (pUser && pUser.binanceApiKey && pUser.binanceSecretKey) {
                try {
                  await closeBinancePosition(
                    ut.symbol,
                    pUser.binanceApiKey,
                    pUser.binanceSecretKey,
                  );
                  console.log(
                    `[Binance-Real] Auto-closed ${ut.side} ${ut.symbol} for user ${ut.userId}`,
                  );
                } catch (e: any) {
                  console.error(
                    `[Binance-Real close error] User ${ut.userId} Symbol ${ut.symbol}:`,
                    e.message,
                  );
                }
              }
            ...
            await ut.save();
            console.log(
              `[User-Trade] Auto-closed ${ut.side} ${ut.symbol} for user ${ut.userId}: ${outcome.toUpperCase()} ($${realizedPnl.toFixed(2)})`,
            );
          }
        } catch (e) {}
      }
      ...
      try {
        const winningSignals = await TradeSignal.find({ status: "win" })
          .sort({ resolvedAt: -1 })
          .limit(100);
        if (winningSignals.length > 5) {
          // Minimum 5 wins to start leaning
          let sumR2 = 0,
            sumE3 = 0,
            sumR4 = 0;
          let count = 0;
          ...
          if (count > 0) {
            ...
            console.log(
              `[ML-Evaluator] Updated global AI parameters from past ${count} winning trades.`,
            );
          }
        }
      } catch (e) {
  ...
  const runDailyAIOptimizer = async (customPrompt?: string) => {
    ...
    if (
      now - lastOptimizationTime < 24 * 60 * 60 * 1000 &&
    ...
    try {
      console.log(
        "[Daily AI Optimizer] Starting Gemini 3.1 Pro analysis of recent trades to improve win rate... 🧠",
      );
      // Fetch last 100 resolved trades
      const recentTrades = await TradeSignal.find({
        status: { $ne: "pending" },
      })
        .sort({ resolvedAt: -1 })
        .limit(100);
      ...
      if (recentTrades.length < 1) {
        tradeDataMsg =
          "No trades executed yet. Provide default optimized baseline parameters for safe trading.";
      } else {
      ...
      if (aiResponse.text) {
        ...
        if (
          result.retrace2 !== undefined &&
          result.ext3 !== undefined &&
          result.retrace4 !== undefined
        ) {
          await EngineConfig.findOneAndUpdate(
            { id: "global" },
            {
              params: {
                retrace2: result.retrace2,
                ext3: result.ext3,
                retrace4: result.retrace4,
              },
          ...
          console.log(
            "[Daily AI Optimizer] Success! New optimized params generated by Gemini 2.5 Flash.",
            result,
          );
  ...
  app.get("/api/engine/report", async (req: any, res) => {
    if (!isDbConnected)
      return res.json({
        params: null,
        insights: "Database not connected",
        updatedAt: null,
      });
    try {
  ...
  app.post("/api/admin/config", authMiddleware, async (req: any, res) => {
    ...
    try {
      ...
      if (req.body.tradeAmountFixed)
        config.tradeAmountFixed = req.body.tradeAmountFixed;
      if (req.body.telegramUserId)
        config.telegramUserId = req.body.telegramUserId;
      if (req.body.telegramBotToken)
        config.telegramBotToken = req.body.telegramBotToken;
      if (req.body.whatsappNumber)
        config.whatsappNumber = req.body.whatsappNumber;
  ...
  app.post("/api/ml/optimize", authMiddleware, async (req: any, res) => {
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only PRO users and Admins can run the optimizer." });
    }
    // Force it to run
    lastOptimizationTime = 0;
    ...
    res.json({
      success: true,
      message:
        "Daily AI Optimizer triggered via Gemini AI and is running in the background.",
    });
  });
